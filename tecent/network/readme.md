...
- 即将完成使命的HTTP/1、正在走向HTTP/2、面向未来的HTTP/3 以及更安全的https

- 一切静态资源都应该缓存，这是打开页面快速的主要原因
    1. 加速打开页面，做成离线应用
    2. 服务器端省带宽，省去了IO操作。  304  + ETag

HTTP  超文本传输协议  最开始，他就传文本
## HTTP0.9 1991  应答式简单模型
- HTTP 基于TCP，七层协议 三次握手
...
- 浏览器要知道服务器返回的是什么类型，浏览器根据不同的数据类型做针对性的处理
...
- 服务器越来越忙  无应答，  有了状态码
- Cache 机制
- user  Agent 用户代理 (可以查看用户是什么设备)

## HTTP/1.1 
1. 改进持久连接
HTTP/1.1 中增加了持久连接的方法，它的特点是在一个TCP连接上可以传输多个HTTP请求，只要浏览器或服务端没有明确断开连接，那么该TCP连接就会一直保持
同一个域名，默认允许同时建立6个TCP持久连接
2. 不太成熟的HTTP管线化
需要等前面的请求返回之后，才会进行下一次请求，队头阻塞
3. 虚拟主机的支持
一台服务器响应多个域名
Demain ->IP ->服务器+端口-> 服务器端程序
HOST字段，区分不同DOMain 在同一台机器中调用不同的应用
4. Cookie，安全机制

## HTTP 2.0 
  管线优化
  CSS sprite 雪碧图 不在提了 很多图片合到一张上去，